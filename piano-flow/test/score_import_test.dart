...
void main() {
  ...
  Uint8List score(String id) => File('assets/songs/$id.mid').readAsBytesSync();
  ...
  test('the beat can be a note value other than the quarter', () {
    ...
    expect(
      inEighths.lengthInBeats,
      closeTo(inQuarters.lengthInBeats * 2, 1e-6),
    );
  ...
  test('the hands come out of the edition the right way round', () {
    for (final id in ['fur-elise', 'prelude-in-c']) {
      ...
      expect(
        average(song.accompaniment),
        lessThan(average(song.melody)),
        reason: id,
      );
    }
  });
